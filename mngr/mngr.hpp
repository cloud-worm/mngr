...
namespace mngr
{
	...
	// Management and utilities relative to specific paths or files.
	namespace file
	{
		...
		void create_file(const string &_name, bool replace = false)
		{
			if (!replace && file_exists(_name))
			{
				...
				cerr << "Attempted to create file \"" << _name
					 ...
					 \nTry passing 2nd argument (replace) in mngr::file::create_file()"
					 << endl;
				return;
			}
			else
		...
		void remove_line(const string &_name, int line_num = -12)
		{
			...
			if (!file_exists(_name))
			{
				cerr << "Attempted to write to file \"" << _name
					...
					\nmngr::file::create_file() can be called to create it."
					 << endl;
			}
			else
			{
		 ...
		 */
		void replace_text(const string &_name, const string _target, const string _replacement)
		{
			if (!file_exists(_name))
			{
				cerr << "Attempted to write to file \"" << _name
		...
		/**
		 * @brief Get a specific line of a file, as a string.
		 * 
		 * @param _name string: the name of the file.
		 * @param file_line integer: the line of the file to retrieve.
		 * @return Returns the contents of the 
		 */
		string get_line(const string &_name, int file_line = -12)
		{

			if (file_line == -12)
			{
				file_line = mngr::file::count_file_lines(_name);
			}
			else if (file_line <= 0 || file_line > mngr::file::count_file_lines(_name))
			{
				cerr << "You did not input a valid line number!" << endl;
				return "";
			}

			if (mngr::file::file_exists(_name))
			{
				...
				// -1 because vector index begins at 0 and file lines begin at 1
				string _ret = file.at(file_line - 1); // Using at() instead of operator[] as it throws an error

				fin.close();

				// Returns line chosen as string.
				return _ret;
			}
			else
			{
	...
	namespace folder
	{
		...
		{
			// use stats to retrieve stats on the path and get
			struct stat buffer;
	...
	namespace console
	{
		 ...
		 */
		void print(const string &content = "") { cout << content << endl; } // Pretty much a pile of rubbish
		...
		{
			print(content);
			mngr::time::wait(sleeptime); // We have to specify full scope path bc ns's are !=
		}
		...
		{
			// Essentially get input of user with cin stream. ez
			T _input;
	...
	namespace data
	{
		...
		// Many overloads of string converting. Accepts integers all the way to const char *.

		/**
		 * @brief Convert anything into a string.
		 * 
		 * @param input_val type: input number to convert.
		 * @return Returns the input number as a string.
		 */
		string s(int input_val)
		{
			// Stringstream accepts any type. Just get the string generated by the stream
			stringstream ss;
			ss << input_val;
			return ss.str();
		}
